...
from services.common import validate_product_list, shard_index
...
class ClientConnection:
    ...
    async def send_message(self, message: dict):
        if not self.rate_limiter.is_allowed():
            ...
            await self.websocket.send_text(
                rate_limit_msg.model_dump_json(by_alias=True)
            )
            return False
    ...
    async def send_error(self, code: str, message: str):
        ...
        await self.websocket.send_text(error_msg.model_dump_json(by_alias=True))
...
class Gateway:
    def __init__(self, config: dict):
        ...
        # Simple in-memory ring buffer per product for lightweight backfill
        self._ring_max_per_product: int = int(config.get("backfill_ring_size", 2000))
        self._rings: Dict[str, Deque[Tick]] = {}
        # JetStream backfill parameters
        self.backfill_max_messages: int = int(config.get("backfill_max_messages", 5000))
        self.backfill_fetch_batch: int = int(config.get("backfill_fetch_batch", 64))
        self.backfill_fetch_timeout: float = float(
            config.get("backfill_fetch_timeout", 1.0)
        )
        ...
        async def root():
            ...
            <html>
                ...
                <body>
                    ...
                    <p>WebSocket endpoint: <code>ws://localhost:{self.port}/ws</code></p>
                    <h2>Protocol</h2>
                    <h3>Subscribe</h3>
    ...
    async def _handle_client_message(self, client: ClientConnection, data: str):
        try:
            ...
            operation = message.get("operation") or message.get("op")
            ...
            if operation == "subscribe":
                ...
                await client.send_error(
                    "INVALID_OPERATION", f"Unknown operation: {operation}"
                )
    ...
    async def _handle_subscribe(self, client: ClientConnection, message: dict):
                    ...
                    try:
                        ...
                        if row:
                            ...
                            await client.send_message(
                                snapshot_msg.model_dump(by_alias=True)
                            )
                            sent = True
                    except Exception as e:
                        print(
        ...
        # After snapshots, perform JetStream backfill grouped by shard
        try:
            req = SubscribeRequest.model_validate(message)
            if req.from_seq:
                shard_map: Dict[int, Dict[str, int]] = {}
                for product in products:
                    if product in req.from_seq:
                        sid = shard_index(product, self.num_shards)
                        shard_map.setdefault(sid, {})[product] = int(
                            req.from_seq[product]
                        )
                for sid, fseq_map in shard_map.items():
                    ticks = await self.broker.backfill_since(
                        shard=sid,
                        from_seq=fseq_map,
                        products=list(fseq_map.keys()),
                        max_messages=self.backfill_max_messages,
                        fetch_batch=self.backfill_fetch_batch,
                        fetch_timeout=self.backfill_fetch_timeout,
                    )
                    for t in ticks:
                        await client.send_message(
                            IncrMessage(data=t).model_dump(by_alias=True)
                        )
        except Exception as e:
    ...
    async def _handle_ping(self, client: ClientConnection, message: dict):
        try:
            ...
            await client.send_message(pong_msg.model_dump(by_alias=True))
    ...
    async def _broadcast_tick(self, tick: Tick):
        ...
        message = incr_msg.model_dump(by_alias=True)

        # Append to ring for backfill support
        ring = self._rings.get(tick.product)
        if ring is None:
            ring = deque(maxlen=self._ring_max_per_product)
            self._rings[tick.product] = ring
        ring.append(tick)
    ...
    async def _send_backfill(
        self, client: ClientConnection, product: str, from_seq: int
    ) -> None:
        """Send available backfill from the recent in-memory ring.

        This is a best-effort approach until JetStream-based backfill by sequence is implemented.
        """
        ring = self._rings.get(product)
        if not ring:
            return
        for tick in ring:
            if tick.seq > from_seq:
                msg = IncrMessage(data=tick).model_dump(by_alias=True)
