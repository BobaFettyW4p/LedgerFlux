            ...
            while True:
                try:
                    msgs = await asyncio.wait_for(
                        psub.fetch(10, timeout=1.0), timeout=2.0
                    )
                    ...
                    for msg in msgs:
                        try:
                            ...
                            print(
                                f"Received message: {tick.product} - {tick.type} (seq: {tick.seq})"
                            )
    ...
    def _validate_tick(self, tick: Tick) -> bool:
        try:
            ...
            if (
                ...
                print(
                    f"Invalid spread: bid={tick.fields.best_bid.px}, ask={tick.fields.best_ask.px}"
                )
                return False
            ...
            if tick.product in self.last_sequences:
                if tick.seq < self.last_sequences[tick.product]:
                    print(
                        f"Out-of-order sequence: {tick.product} {tick.seq} < {self.last_sequences[tick.product]}"
                    )
