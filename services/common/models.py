...
class SubscribeRequest(BaseModel):
    operation: str = Field("subscribe", description="Operation type")
...
class UnsubscribeRequest(BaseModel):
    operation: str = Field("unsubscribe", description="Operation type")
    products: list[str] = Field(description="List of products to unsubscribe from")


class PingRequest(BaseModel):
    operation: str = Field("ping", description="Operation type")
    timestamp: int = Field(description="Timestamp")


class SnapshotMessage(BaseModel):
    operation: str = Field("snapshot", description="Operation type")
    data: Snapshot = Field(description="Snapshot data")


class IncrMessage(BaseModel):
    operation: str = Field("incr", description="Operation type")
    data: Tick = Field(description="Tick data")


class RateLimitMessage(BaseModel):
    operation: str = Field("rate_limit", description="Operation type")
    retry_ms: int = Field(description="Retry delay in milliseconds")


class PongMessage(BaseModel):
    operation: str = Field("pong", description="Operation type")
    timestamp: int = Field(description="Timestamp from ping")


class ErrorMessage(BaseModel):
    operation: str = Field("error", description="Operation type")
