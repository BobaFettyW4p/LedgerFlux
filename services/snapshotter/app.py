...
class Snapshotter:
    def __init__(self, config: Dict[str, Any]):
        ...
        self.app = FastAPI(title="Snapshotter")
        ...
        async def ready():
            ...
            return {
                "status": "ready"
                if (broker_connected and self._store_ready)
                else "not_ready",
    ...
    async def start(self):
        ...
        # Start HTTP server first so probes pass early
        print("Starting HTTP health server on :8082")
        http_cfg = uvicorn.Config(self.app, host="0.0.0.0", port=8082, log_level="info")
    ...
    async def _process_messages(self):
        ...
        await self.broker.subscribe_to_shard(
            self.shard_id, message_handler, consumer_name=consumer_name
        )

        # Keep the coroutine alive by awaiting the fetch task for this shard
        task = self.broker.fetch_tasks.get(self.shard_id)
        if task is not None:
            await task
        else:
            # Fallback: block forever
