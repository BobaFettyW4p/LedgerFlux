...
class TestPostgresSnapshotStore:
    ...
    @patch.dict(
        os.environ, {"PG_DSN": "postgresql://env:pass@envhost:5432/envdb"}, clear=True
    )
    ...
    async def test_connect_first_time(self, mock_connect, mock_async_connection):
        ...
        mock_connect.assert_called_once_with(
            "postgresql://test:test@localhost:5432/test", autocommit=True
        )
    ...
    async def test_ensure_schema_creates_table(
        self, mock_connect, mock_async_connection, mock_async_cursor
    ):
    ...
    @patch("services.common.pg_store.psycopg.AsyncConnection.connect")
    async def test_ensure_schema_when_already_connected(
        self, mock_connect, mock_async_connection, mock_async_cursor
    ):
    ...
    @patch("services.common.pg_store.psycopg.AsyncConnection.connect")
    async def test_upsert_latest_basic(
        self, mock_connect, mock_async_connection, mock_async_cursor
    ):
    ...
    @patch("services.common.pg_store.psycopg.AsyncConnection.connect")
    async def test_upsert_latest_with_datetime_serialization(
        self, mock_connect, mock_async_connection, mock_async_cursor
    ):
        ...
        state = {"last_trade": {"px": 50000.0, "qty": 0.5}, "last_update": dt}
    ...
    @patch("services.common.pg_store.psycopg.AsyncConnection.connect")
    async def test_upsert_latest_without_datetime(
        self, mock_connect, mock_async_connection, mock_async_cursor
    ):
    ...
    @patch("services.common.pg_store.psycopg.AsyncConnection.connect")
    async def test_upsert_latest_connects_if_needed(
        self, mock_connect, mock_async_connection, mock_async_cursor
    ):
    ...
    @patch("services.common.pg_store.psycopg.AsyncConnection.connect")
    async def test_get_latest_found(
        self, mock_connect, mock_async_connection, mock_async_cursor
    ):
    ...
    @patch("services.common.pg_store.psycopg.AsyncConnection.connect")
    async def test_get_latest_connects_if_needed(
        self, mock_connect, mock_async_connection, mock_async_cursor
    ):
